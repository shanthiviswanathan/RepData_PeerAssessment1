...
maxInt2 <- dtIntervals[interval,max(meanSteps)]
```
...
mergedDt[is.na(steps), steps:=steps.1]
newDt <- mergedDt[,list(interval, steps, date)]

## Aggregate imputed data by day
newDtDailySteps <- aggregate(steps ~ date, data=newDt, FUN=sum)

## Mean and median
```{r, results='asis'}
newDtDailySteps[, list(mean = mean(steps, na.rm=TRUE), median = median(steps, na.rm=TRUE))]
```
